...
const ClassOrientationQuiz = () => {
    ...
    useEffect(() => {
        setTopicSlug("quiz-2-course-1");
    }, []);

    const nextPage = () => {
        setIsLoading(true);
        navigate(`/class/${courseId}/organization`)
    }
                                    ...
                                    <>
                                        <div key={index} className='w-full h-fit flex flex-row justify-between items-center p-6 border border-[var(--color-secondary)] rounded-full'>
                                            <div className='flex flex-row items-center justify-start gap-4'>
                                                ...
                                                <h3 className='text-[20px] text-left leading-none font-semibold'>Soal No. {i.id}</h3>
                                            </div>
                                        ...
                                        <div className='w-full h-fit grid grid-cols-2'>
                                            ...
                                            <div className='w-full h-full flex flex-col gap-4 p-6'>
                                                <h3 className='w-fit text-[36px] text-left font-semibold'>{i.question}</h3>
                                                {
                                                    option.map((i, index) => {
                                                        return (
                                                            <>
            ...
            <section className='w-full h-fit flex flex-col justify-center items-center p-6'>
                <div className='w-full h-full flex flex-col justify-between text-[var(--color-secondary)] rounded-3xl p-6 gap-12'>
                    ...
                    <div className='w-full h-fit flex items-center justify-between'>
                        <button onClick={() => navigate('/class/orientation')} className='w-fit h-auto flex items-center justify-center gap-2 bg-[var(--color-tertiary)] text-[var(--color-secondary)] rounded-full pt-[8px] pb-[10px] px-[20px] opacity-80 hover:opacity-100'>
